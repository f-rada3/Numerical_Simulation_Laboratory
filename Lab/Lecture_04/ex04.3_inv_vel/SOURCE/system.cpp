...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    double vx,vy,vz;
    ifstream vel_in;
    vel_in.open("../INPUT/CONFIG/velocities.out");  
    if(vel_in.is_open()){
      for(int i=0; i<_npart; i++){
        vel_in >> vx >> vy >> vz;
        // - means that I'm going backward
    ...
    // con restart = 0
    // Implementation of a velocity distribution represented by a Dirac delta centered at v_T
    // Ensure 3D
    if(_ndim != 3){
  ...
  if (input.is_open()){
    while ( input >> property ){
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutpv.close();
        input>>_n_bins_v;                                 // It reads nbins(30) from properties.dat
        _nprop += _n_bins_v;                              // Measure of N_bin values (a lot of bins - n_prop++ isn't enough)
        _bin_size_v = 4*sqrt(_temp) / (double)_n_bins_v;  // Equipartition theorem (4-factor: make v values stay
                                                          // in the v range choosen [4-times average velocity])
        _measure_pofv = true;
        _index_pofv = index_property;                     // Saves initial pos in vector of proprerties for P(v) histogram
                                                          // INDEX POFV says that first bin starts from this position
        index_property += _n_bins_v;                      // Makes advance index_property of a # of positions = # bins
                                                          // P(v) occupies _n_bins_v consecutive cells
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    // AGGIUNGO SALVATAGGIO VELOCITA' FINALI
    coutf.open("../INPUT/CONFIG/velocities.out");
    if(coutf.is_open()){
      for(int i=0; i<_npart; i++){
        coutf << setprecision(17)
              << _particle(i).getvelocity(0) << "   "
              << _particle(i).getvelocity(1) << "   "
              << _particle(i).getvelocity(2) << endl;
      }
...
void System :: read_configuration(){
    ...
    if(!_restart){
        ...
        if(cinf.is_open()){
            ...
            for(int i=0; i<_npart; i++){
                cinf >> particle >> x >> y >> z;
                _particle(i).setposition(0, this->pbc(_halfside(0)*x, 0));      // The coordinates read from config.fcc are rescaled by the full box side _side,
                _particle(i).setposition(1, this->pbc(_halfside(1)*y, 1));      // so the particles are initially placed over the whole simulation box.
                _particle(i).setposition(2, this->pbc(_halfside(2)*z, 2));      // For Exercise 04.2, instead, the initial FCC configuration must occupy only
                _particle(i).acceptmove(); // _x_old = _x_new                   // half of the box in each spatial direction, so the rescaling factor should
                                                                                // be _halfside rather than _side.
              }
            } else {
              cerr << "PROBLEM: Unable to open INPUT file config.fcc" << endl;
              exit(EXIT_FAILURE);
            }
      } else { //caso restart=1
...
void System :: measure(){ // Measure properties
  ...
  // MODIFICA ESERCIZIO 4 -> Velocities (module) distribution
  // POFV //////////////////////////////////////////////////////////////////////
  if (_measure_pofv){     // Execute only if requested
    double v_x, v_y, v_z;
    for(int i = 0; i < _npart; i ++){
