...
void System :: initialize_velocities(){
  ...
  if(_restart){
  ...
  } else { // RESTART = 0: special low-entropy initial velocities
    ...
    for (int i=0; i<_npart; i++){
      ...
      // Initialize only one velocity component for each particle
      int dir = int(_rnd.Rannyu(0.0,3.0));   // 0, 1, 2
      double sign;
...
void System :: write_configuration(){
  ...
  coutf.open("../OUTPUT/CONFIG/velocities.out"); //salvo velocità finali
  if(coutf.is_open()){
      for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
    ...
    if(!_restart){
        ...
        if(cinf.is_open()){
            ...
            for(int i=0; i<_npart; i++){
                cinf >> particle >> x >> y >> z;
                _particle(i).setposition(0, this->pbc(0.5*_side(0)*x, 0));      // The coordinates read from config.fcc are rescaled by the full box side _side,
                _particle(i).setposition(1, this->pbc(0.5*_side(1)*y, 1));      // so the particles are initially placed over the whole simulation box.
                _particle(i).setposition(2, this->pbc(0.5*_side(2)*z, 2));      // For Exercise 04.2, instead, the initial FCC configuration must occupy only
                _particle(i).acceptmove(); // _x_old = _x_new                   // half of the box in each spatial direction, so the rescaling factor should
                                                                      // be 0.5 * _side rather than _side.
              }
            } else {
