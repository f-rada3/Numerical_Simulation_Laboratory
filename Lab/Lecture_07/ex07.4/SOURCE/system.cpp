  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8.0/3.0)*((M_PI*_rho/(3*pow(_r_cut,9)))- (M_PI*_rho/pow(_r_cut,3))); //To be fixed in exercise 7.1
        ...
        index_property++;
        _ptail = 32.0 * M_PI * _rho * (1.0 / (9.0 * pow(_r_cut, 9)) - 1.0 / (6.0 * pow(_r_cut, 3)));  //To be fixed in exercise 7.1
      } else if( property == "GOFR" ){

        //MODIFICATO IN ESERCITAZIONE 7
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << setw(12) << "BLOCK"
               << setw(12) << "DISTANCE"
               << setw(12) << "ACTUAL_GOFR"
               << setw(12) << "AVE_GOFR"
               << setw(12) << "ERROR" << endl;
        ...
        input>>_n_bins_g;
        _nprop+=_n_bins_g;
        _bin_size_g = (_halfside.min() )/(double)_n_bins_g;
        _measure_gofr = true;
        _index_gofr = index_property;
        index_property+= _n_bins_g;
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.fcc"); //Ho sostituito config.xyz con config.fcc
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: block_reset(int blk){ // Reset block accumulators to zero
  ...
  _block_av.zeros();
  // _naccepted=0.0;
  // _nattempts=0.0; //CAMBIARE ASSOLUTAMENTE: reset acceptance
  return;
}
      ...
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
          for(int k=1; k<_n_bins_g+1; k++){
            ...
            double delta_v = (4.0/3.0) * M_PI * (pow(r_upper,3) - pow(r_lower,3));  //crosta di volume, serve per normalizzare il numero di coppie che cadono in quel bin
  ...
  if (_measure_kenergy){
    if(_sim_type == 0){
     for (int i=0; i<_npart; i++)
      ...
      kenergy_temp = _npart * 0.5 * 3.0 * _temp;
    }
