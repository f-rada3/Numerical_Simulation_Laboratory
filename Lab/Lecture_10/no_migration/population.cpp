...
Individual Population::SelectRank(){
    ...
    if(index < 0 || index >= _npop){
    cerr << "SelectRank out of bounds: index = " << index << endl;
    exit(1);
}
...
double Population::BestFitness(){
    if(!_individuals.empty()){
        return _individuals[0].GetFitness();
    } else {
        cerr << "Population is empty. Cannot get best fitness." << endl;
        return -1; // or throw an exception
    }
}

void Population::InitializeByFile(Random & rnd, const string & filename, int num_of_cities, int npop, int ngen, double mut_prob, double p_exp, int rank) {
    _rnd = &rnd;
    _type = "Italy";
    _num_of_cities = num_of_cities;
    _npop = npop;
    ...
    // creazione file output
    _loss.open(_type + "_loss_rank_" + to_string(rank) + ".dat");
}

void Population::EvaluateImmigrant(const arma::uvec & path){
    // l'individuo peggiore viene scambiato con uno buono
    // da un vicino
    _individuals.back().SetPath(path);
    // devo fare di nuovo il sort
    FitnessSorting();
}

void Population::Closure(int rank){
    if(_loss.is_open()) _loss.close();

    // estrazione genoma del miglior individuo
    uvec best_path = _individuals[0].GetPath();

    ofstream path(_type + "_best_path_rank_" + to_string(rank) + ".dat");
