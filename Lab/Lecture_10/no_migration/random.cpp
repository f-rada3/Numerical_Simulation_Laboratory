...
// Method to generate a random angle in (0,\pi) without using the actual pi value
double Random :: Angle(){
   double x, y, d;
   do{
      x = Rannyu(-1.,1.);
...
// Method to generate a random variable with distribution p(x)=-2x+2
...
double Random :: Rannyu(void){
  ...
  // check if r is in the valid range [0,1)
  if(r < 0.0 || r >= 1.0){
      cerr << "RNG ERROR: r = " << r
           << " l1=" << l1 << " l2=" << l2
           << " l3=" << l3 << " l4=" << l4
